...
""".format(sc_name)
    ...
    await retry_async_with_timeout(verify_completed,
                                   (master,
                                    'po',
                                    ['{}-write-test'.format(sc_name)]),
                                   timeout_msg="Unable to create write"
                                               " pod for ceph test")
...
""".format(sc_name)
    ...
    await retry_async_with_timeout(verify_completed,
                                   (master,
                                    'po',
                                    ['{}-read-test'.format(sc_name)]),
                                   timeout_msg="Unable to create write"
                                               " pod for ceph test")

    output = await master.run(
        '/snap/bin/kubectl logs {}-read-test'.format(sc_name))
    assert output.status == 'completed'
